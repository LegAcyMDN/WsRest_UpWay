...
namespace WsRest_UpWay.Models.DataManager
{
    public class CaracteristiqueVeloManager : IDataRepository<CaracteristiqueVelo>
    {
        ...
        public CaracteristiqueVeloManager()
        {
        }

        public CaracteristiqueVeloManager(S215UpWayContext context)
        {
        ...
        {
            return await upwaysDbContext.Caracteristiquevelos.Skip(page * PAGE_SIZE).Take(PAGE_SIZE).ToListAsync();
        }

        public async Task<ActionResult<CaracteristiqueVelo>> GetByIdAsync(int id)
        {
            return await upwaysDbContext.Caracteristiquevelos.FirstOrDefaultAsync(a => a.CaracteristiqueVeloId == id);
        }
